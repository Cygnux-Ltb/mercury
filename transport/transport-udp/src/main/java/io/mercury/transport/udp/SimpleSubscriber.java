...
/**
 * A very simple Aeron subscriber application which can receive small
 * non-fragmented messages on a fixed channel and stream ID. The DataHandler
 * method 'printStringMessage' is called when data is received. This application
 * doesn't handle large fragmented messages. For an example of fragmented
 * message reception, see {@link MultipleSubscribersWithFragmentAssembly}.
 */
public class SimpleSubscriber {
    ...
    public static void main(final String[] args) {
        ...
        final FragmentHandler fragmentHandler = (buffer, offset, length, header) -> {
            ...
            System.out.printf("Received message (%s) to stream %d from session %x term id %x term offset %d (%d@%d)%n",
        ...
        // Create an Aeron instance with client-provided context configuration, connect
        // to the
        // media driver, and add a subscription for the given channel and stream using
        // the supplied
        // dataHandler method, which will be called with new messages as they are
        // received.
