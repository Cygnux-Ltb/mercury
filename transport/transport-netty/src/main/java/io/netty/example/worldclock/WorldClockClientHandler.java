...
public class WorldClockClientHandler extends SimpleChannelInboundHandler<LocalTimes> {
	...
	public List<String> getLocalTimes(Collection<String> cities) {
		...
		for (String c : cities) {
			...
			builder.addLocation(Location.newBuilder().setContinent(Continent.valueOf(components[0].toUpperCase()))
		...
		for (LocalTime lt : localTimes.getLocalTimeList()) {
			try (Formatter formatter = new Formatter()) {
				result.add(formatter.format("%4d-%02d-%02d %02d:%02d:%02d %s", lt.getYear(), lt.getMonth(),
						lt.getDayOfMonth(), lt.getHour(), lt.getMinute(), lt.getSecond(), lt.getDayOfWeek().name())
						.toString());
			}
		}
