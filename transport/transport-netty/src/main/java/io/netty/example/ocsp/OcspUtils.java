...
public final class OcspUtils {
     ...
     * <a href="http://www.alvestrand.no/objectid/1.3.6.1.5.5.7.48.1.html">http://www.alvestrand.no/objectid/1.3.6.1.5.5.7.48.1.html</a>
     */
    ...
    /**
     * Returns the OCSP responder {@link URI} or {@code null} if it doesn't have
     * one.
     */
    public static URI ocspUri(X509Certificate certificate) throws IOException {
        ...
        if (!(authorityInfoAccess instanceof DLSequence aiaSequence)) {
            return null;
        }
    ...
    private static <T> T findObject(DLSequence sequence, ASN1ObjectIdentifier oid, Class<T> type) {
        for (ASN1Encodable element : sequence) {
            if (!(element instanceof DLSequence subSequence)) {
                continue;
            }
    ...
    /**
     * TODO: This is a very crude and non-scalable HTTP client to fetch the OCSP
     * response from the CA's OCSP responder server. It's meant to demonstrate the
     * basic building blocks on how to interact with the responder server and you
     * should consider using Netty's HTTP client instead.
     */
