...
public class WorldClockServerHandler extends SimpleChannelInboundHandler<Locations> {
    ...
    public void channelRead0(ChannelHandlerContext ctx, Locations locations) throws Exception {
        ...
        for (Location location : locations.getLocationList()) {
            TimeZone zone = TimeZone.getTimeZone(toString(location.getContinent()) + '/' + location.getCity());
            ...
            builder.addLocalTime(LocalTime.newBuilder().setYear(calendar.get(YEAR)).setMonth(calendar.get(MONTH) + 1)
                    ...
                    .setDayOfWeek(DayOfWeek.valueOf(calendar.get(DAY_OF_WEEK))).setHour(calendar.get(HOUR_OF_DAY))
                    .setMinute(calendar.get(MINUTE)).setSecond(calendar.get(SECOND)).build());
        }
