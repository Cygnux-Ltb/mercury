...
public class LoadBalancingBroker {
	 ...
	 * conceptually acts as a separate process. This is the worker task, using a REQ
	 * socket to do load-balancing.
	 */
	 ...
	 * requests between the two layers. Workers signal READY when they start; after
	 * that we treat them as ready when they reply with a response back to a client.
