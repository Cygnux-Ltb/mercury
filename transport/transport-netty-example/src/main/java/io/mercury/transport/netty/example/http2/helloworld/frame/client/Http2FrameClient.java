...
/**
 * An HTTP2 client that allows you to send HTTP2 frames to a server using the
 * newer HTTP2 approach (via
 * {@link io.netty.handler.codec.http2.Http2FrameCodec}). When run from the
 * command-line, sends a single HEADERS frame (with prior knowledge) to the
 * server configured at host:port/path. You should include
 * {@link io.netty.handler.codec.http2.Http2ClientUpgradeCodec} if the HTTP/2
 * server you are hitting doesn't support h2c/prior knowledge.
 */
