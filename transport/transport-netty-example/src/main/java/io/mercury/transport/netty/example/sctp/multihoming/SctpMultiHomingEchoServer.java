...
import io.mercury.transport.netty.example.sctp.SctpEchoServerHandler;
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			b.group(bossGroup, workerGroup).channel(NioSctpServerChannel.class).option(ChannelOption.SO_BACKLOG, 100)
					.handler(new LoggingHandler(LogLevel.INFO)).childHandler(new ChannelInitializer<SctpChannel>() {
						@Override
