...
public final class Html {
	...
	public static byte[] body(int latency) {
		...
		// The string to be built contains 13192 fixed characters plus the variable
		// latency and random cache-bust.
		...
		for (int y = 0; y < IMAGES_Y_AXIS; y++) {
			for (int x = 0; x < IMAGES_X_AXIS; x++) {
				sb.append("<img width=30 height=29 src='/http2?x=").append(x).append("&y=").append(y)
						.append("&cachebust=").append(r).append("&latency=").append(latency).append("'>");
			}
			sb.append("<br/>\r\n");
		}
		sb.append("</div>");
