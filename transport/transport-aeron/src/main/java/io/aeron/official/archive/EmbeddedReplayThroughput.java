...
public class EmbeddedReplayThroughput extends EmbeddedReplayThroughputValue implements AutoCloseable {
    ...
    public static void main(final String[] args) {
        ...
        try (EmbeddedReplayThroughput test = new EmbeddedReplayThroughput()) {
            ...
            do {
                ...
                System.out.printf("Replayed %.02f MB @ %.02f MB/s - %,d msg/sec - %d byte payload + 32 byte header%n",
    ...
    EmbeddedReplayThroughput() {
        ...
        archivingMediaDriver = ArchivingMediaDriver.launch(new MediaDriver.Context().dirDeleteOnStart(true),
                new Archive.Context().archiveDir(archiveDir).recordingEventsEnabled(false));
    ...
    private void replayRecording(final long recordingLength, final long recordingId) {
        try (Subscription subscription = aeronArchive.replay(recordingId, 0L, recordingLength, REPLAY_URI,
                REPLAY_STREAM_ID)) {
