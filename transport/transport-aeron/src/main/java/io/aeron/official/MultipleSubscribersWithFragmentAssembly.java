...
/**
 * A subscriber application with two subscriptions which can receive fragmented
 * messages.
 * <p>
 * Creates two subscriptions on a given channel subscribed to two different
 * stream IDs. The default STREAM_ID and CHANNEL are configured in
 * {@link SampleConfiguration}. The default channel and stream IDs can be
 * changed by setting Java system properties at the command line, e.g.:
...
public class MultipleSubscribersWithFragmentAssembly {
    ...
    /**
     * Return a reusable, parameterized {@link FragmentHandler} that prints to
     * stdout for the first stream(STREAM).
     *
    ...
    public static FragmentHandler reassembledMessage1(final int streamId) {
        return (buffer, offset, length, header) -> {
            System.out.format("message to stream %d from session %x term id %x term offset %d (%d@%d)%n", streamId,
                    header.sessionId(), header.termId(), header.termOffset(), length, offset);

            if (length != 10000) {
                System.out.format("Received message was not assembled properly;"
                        + " received length was %d, but was expecting 10000%n", length);
            }
        };
    }

    /**
     * A reusable, parameterised {@link FragmentHandler} that prints to stdout for
     * the second stream (STREAM + 1).
     *
     * @param streamId to show when printing.
     * @return subscription data handler function that prints the message contents.
     */
