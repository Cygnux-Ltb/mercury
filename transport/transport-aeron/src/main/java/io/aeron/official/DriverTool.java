...
import static io.aeron.CncFileDescriptor.pid;
import static io.aeron.CncFileDescriptor.startTimestampMs;

/**
 * Tool for printing out Aeron Media Driver Information. A command-and-control
 * (CnC) file is maintained by a media driver in shared memory. This application
 * reads the CnC file and prints its status. Layout of the Cnc file is described
 * in {@link CncFileDescriptor}.
 */
public class DriverTool {
    ...
    private static void printDateActivityAndStartTimestamps(final long startTimestamp, final long activityTimestamp) {
        System.out.format("%1$tH:%1$tM:%1$tS (start: %2tF %2$tH:%2$tM:%2$tS, activity: %3tF %3$tH:%3$tM:%3$tS)%n",
                new Date(), new Date(startTimestamp), new Date(activityTimestamp));
    }

    private static void checkForHelp(final String[] args) {
        for (final String arg : args) {
            if ("-?".equals(arg) || "-h".equals(arg) || "-help".equals(arg)) {
                System.out.format("Usage: [-Daeron.dir=<directory containing CnC file>] DriverTool <pid> <terminate>%n"
                        + "    pid: prints PID of driver only%n" + "    terminate: request the driver to terminate%n");
                System.out.flush();
                System.exit(0);
            }
        }
