...
/**
 * Tool for printing out Aeron counters. A command-and-control (CnC) file is
 * maintained by media driver in shared memory. This application reads the
 * cnc file and prints the counters. Layout of the cnc file is described in
 * {@link CncFileDescriptor}.
 * <p>
 * This tool accepts filters on the command line, e.g. for connections only see
 * example below:
 * <p>
 * <code>
...
public class AeronStat {
    ...
    /**
     * The identity of each counter that can either be the system counter id or
     * registration id for positions.
     */
    ...
    public static void main(final String[] args) throws IOException, InterruptedException {
        ...
        if (0 != args.length) {
            ...
            for (final String arg : args) {
                ...
                switch (argName) {
                    case WATCH -> watch = Boolean.parseBoolean(argValue);
                    case DELAY -> delayMs = Long.parseLong(argValue) * 1000L;
                    case COUNTER_TYPE_ID -> typeFilter = Pattern.compile(argValue);
                    case COUNTER_IDENTITY -> identityFilter = Pattern.compile(argValue);
                    case COUNTER_SESSION_ID -> sessionFilter = Pattern.compile(argValue);
                    case COUNTER_STREAM_ID -> streamFilter = Pattern.compile(argValue);
                    case COUNTER_CHANNEL -> channelFilter = Pattern.compile(argValue);
                    default -> {
        ...
        final CounterFilter counterFilter = new CounterFilter(typeFilter, identityFilter, sessionFilter, streamFilter,
                channelFilter);
    ...
    private static void printOutput(final CncFileReader cncFileReader, final CounterFilter counterFilter) {
        ...
        System.out.println(" - Aeron Stat (CnC v" + cncFileReader.semanticVersion() + ")" + ", pid "
                + SystemUtil.getPid() + ", heartbeat age " + cncFileReader.driverHeartbeatAgeMs() + "ms");
        System.out.println("======================================================================");
        ...
        for (final String arg : args) {
            if ("-?".equals(arg) || "-h".equals(arg) || "-help".equals(arg)) {
                System.out.format("Usage: [-Daeron.dir=<directory containing CnC file>] AeronStat%n"
                        + "\t[delay=<seconds between updates>]%n" + "\t[watch=<true|false>]%n"
                        + "filter by optional regex patterns:%n" + "\t[type=<pattern>]%n" + "\t[identity=<pattern>]%n"
                        + "\t[session=<pattern>]%n" + "\t[stream=<pattern>]%n" + "\t[channel=<pattern>]%n");
    ...
    static class CounterFilter {
        ...
        CounterFilter(final Pattern typeFilter, final Pattern identityFilter, final Pattern sessionFilter,
                      final Pattern streamFilter, final Pattern channelFilter) {
        ...
        private static boolean match(final Pattern pattern, final Supplier<String> supplier) {
            return null == pattern
                    || pattern.matcher(supplier.get()).find();
        }
        ...
        boolean filter(final int typeId, final DirectBuffer keyBuffer) {
            ...
            if (SYSTEM_COUNTER_TYPE_ID == typeId
                ...
                return match(identityFilter, () -> Long.toString(keyBuffer.getLong(REGISTRATION_ID_OFFSET)))
