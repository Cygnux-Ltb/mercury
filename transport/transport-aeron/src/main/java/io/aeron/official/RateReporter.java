...
public final class RateReporter extends RateReporterRhsPadding implements Runnable {
    ...
    /**
     * Create a rate reporter with the given report interval in nanoseconds and the
     * reporting function.
     *
    ...
    public void run() {
        do {
            ...
            final double messagesPerSec = ((currentTotalMessages - lastTotalMessages) * (double) reportIntervalNs)
                    / (double) timeSpanNs;
            final double bytesPerSec = ((currentTotalBytes - lastTotalBytes) * (double) reportIntervalNs)
                    / (double) timeSpanNs;
