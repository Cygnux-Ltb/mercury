...
/**
 * Latency test using a ping-pong approach to measure RTT and store all results
 * in a {@link Histogram}.
 */
public class EmbeddedPingPong {
    ...
    public static void main(final String[] args) throws InterruptedException {
        ...
        final MediaDriver.Context ctx = new MediaDriver.Context().threadingMode(ThreadingMode.DEDICATED)
                .conductorIdleStrategy(new BackoffIdleStrategy(1, 1, 1000, 1000))
                .receiverIdleStrategy(NoOpIdleStrategy.INSTANCE).senderIdleStrategy(NoOpIdleStrategy.INSTANCE);

        try (MediaDriver ignored = MediaDriver.launch(ctx); Aeron aeron = Aeron.connect()) {
    ...
    private static void runPing(final Aeron aeron) throws InterruptedException {
        ...
        try (Subscription pongSubscription = aeron.addSubscription(PONG_CHANNEL, PONG_STREAM_ID,
                EmbeddedPingPong::availablePongImageHandler, null);
            ...
            System.out.format("Warming up... %d iterations of %,d messages%n", WARMUP_NUMBER_OF_ITERATIONS,
    ...
    private static Thread startPong(final Aeron aeron) {
        return new Thread(() -> {
            ...
            try (Subscription pingSubscription = aeron.addSubscription(PING_CHANNEL, PING_STREAM_ID);
                ...
                final FragmentHandler fragmentHandler = (buffer, offset, length, header) -> pingHandler(bufferClaim,
                        pongPublication, buffer, offset, length, header);
    ...
    private static void roundTripMessages(final FragmentHandler fragmentHandler, final Publication pingPublication,
                                          final Subscription pongSubscription, final long numMessages) {
    ...
    private static void pongHandler(final DirectBuffer buffer, final int offset, final int length,
                                    final Header header) {
    ...
    public static void pingHandler(final BufferClaim bufferClaim, final Publication pongPublication,
