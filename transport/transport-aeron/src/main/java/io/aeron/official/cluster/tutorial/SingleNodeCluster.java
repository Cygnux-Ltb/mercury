 ...
 * Includes a simple service to show event processing, and includes a cluster
 * client.
 * <p>
...
public class SingleNodeCluster implements AutoCloseable {
    ...
    static class Service implements ClusteredService {
        ...
        public void onTimerEvent(final long correlationId, final long timestamp) {
            ...
            cluster.forEachClientSession((clientSession) -> {
                idleStrategy.reset();
    ...
    public SingleNodeCluster(final ClusteredService externalService, final boolean cleanStart) {
        ...
        mediaDriverContext.threadingMode(ThreadingMode.SHARED).errorHandler(Throwable::printStackTrace)
                .dirDeleteOnShutdown(true).dirDeleteOnStart(true);

        archiveContext.recordingEventsEnabled(false).threadingMode(ArchiveThreadingMode.SHARED)
                .deleteArchiveOnStart(cleanStart);

        consensusModuleContext.errorHandler(Throwable::printStackTrace).deleteDirOnStart(cleanStart);
    ...
    void connectClientToCluster() {
        ...
        clientMediaDriver = MediaDriver.launch(new MediaDriver.Context().threadingMode(ThreadingMode.SHARED)
                .dirDeleteOnStart(true).dirDeleteOnShutdown(true).errorHandler(Throwable::printStackTrace)
                .aeronDirectoryName(aeronDirectoryName));

        client = AeronCluster.connect(new AeronCluster.Context().errorHandler(Throwable::printStackTrace)
                .egressListener(egressMessageListener).aeronDirectoryName(aeronDirectoryName));
    }
    ...
    void takeSnapshot() {
        ...
        final AtomicCounter controlToggle = ClusterControl.findControlToggle(
                clusteredMediaDriver.mediaDriver().context().countersManager(), consensusModuleContext.clusterId());
