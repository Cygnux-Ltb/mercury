...
public class SimpleServerAndClientTest extends NetworkTestCommon {
	...
	static {
		CommonLogConfigurator.setFolder("runtime");
		CommonLogConfigurator.setFilename("socket-test");
	...
	public void test() throws IOException {
		...
		for (;;) {
			...
			EventLoop eg = new EventGroup(true)) {
				...
				// an example message that we are going to send from the server to the client
				// and back
				@NotNull
				...
				try (@NotNull
				TcpChannelHub tcpChannelHub = createClient(eg, desc)) {

					// create the message the client sends to the server

					// the tid must be unique, its reflected back by the server, it must be at the
					// start
					// of each message sent from the server to the client. Its use by the client to
					// identify which
					...
					tcpChannelHub.lock2(() -> tcpChannelHub.writeSocket(wire, true, false), true,
							TryLock.TRY_LOCK_WARN);

					// read the reply from the socket ( timeout after 5 second ), note: we have to
					// pass
					// the tid
					try {
						...
						// running to read this, for example if you commented out "createServer(desc,
						// eg); "
	...
	private TcpChannelHub createClient(@NotNull EventLoop eg, String desc) {
		return new TcpChannelHub(null, eg, WireType.TEXT, "/", uri(desc), false, null, HandlerPriority.TIMER,
				new FatalFailureConnectionStrategy(3, false));
	}
