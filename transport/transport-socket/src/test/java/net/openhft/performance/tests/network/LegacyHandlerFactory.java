...
public enum LegacyHandlerFactory {

    INSTANCE;

    public static <T extends NetworkContext<T>> Function<T, TcpEventHandler<T>> legacyTcpEventHandlerFactory(
            @NotNull final Function<T, TcpHandler<T>> defaultHandedFactory, final long heartbeatIntervalTicks,
        ...
        return (networkContext) -> {
            ...
            @NotNull final HeaderTcpHandler<T> headerTcpHandler = new HeaderTcpHandler<>(handler, consumer);

            @NotNull final WireTypeSniffingTcpHandler<T> wireTypeSniffingTcpHandler = new WireTypeSniffingTcpHandler<>(handler,
                    (nc) -> headerTcpHandler);
    ...
    public static <T extends NetworkContext<T>> Function<T, TcpEventHandler<T>> simpleTcpEventHandlerFactory(
            @NotNull final Function<T, TcpHandler<T>> defaultHandedFactory, final WireType text) {
        return (networkContext) -> {

            networkContext.wireOutPublisher(new VanillaWireOutPublisher(TEXT));
            @NotNull final TcpEventHandler<T> handler = new TcpEventHandler<>(networkContext);
            handler.tcpHandler(new WireTypeSniffingTcpHandler<>(handler, defaultHandedFactory));
            return handler;

        };
    }

    public static <T extends NetworkContext<T>> Function<T, TcpEventHandler<T>> defaultTcpEventHandlerFactory(
            @NotNull final Function<T, TcpHandler<T>> defaultHandedFactory) {
        return (networkContext) -> {
            networkContext.wireOutPublisher(new VanillaWireOutPublisher(TEXT));
            @NotNull final TcpEventHandler<T> handler = new TcpEventHandler<>(networkContext);
