...
public class PingPongWithMains {
	...
	private static void testLatency(String desc, @NotNull Function<Bytes<?>, Wire> wireWrapper,
		...
		Bytes<ByteBuffer> outBytes = Bytes.wrapForWrite(out);
		Wire outWire = wireWrapper.apply(outBytes);

		ByteBuffer in = ByteBuffer.allocateDirect(64 * 1024);
		Bytes<ByteBuffer> inBytes = Bytes.wrapForRead(in);
		...
		for (int i = -12000; i < tests; i++) {
			...
			for (@NotNull
			ChronicleSocketChannel socket : sockets) {
				out.clear();
				outBytes.clear();
		...
		Arrays.sort(times);
		System.out.printf(
				"%s: Loop back echo latency was %.1f/%.1f %,d/%,d %,d/%d us for 50/90 99/99.9 99.99/worst %%tile%n",
				desc, times[times.length / 2] / 1e3, times[times.length * 9 / 10] / 1e3,
				times[times.length - times.length / 100] / 1000, times[times.length - times.length / 1000] / 1000,
				times[times.length - times.length / 10000] / 1000, times[times.length - 1] / 1000);
	}
	...
	public void testServer() throws IOException {
		...
		EventLoop eg = EventGroup.builder().withDaemon(true)
			...
			@NotNull
			AcceptorEventHandler<?> eah = new AcceptorEventHandler(serverHostPort,
					simpleTcpEventHandlerFactory(EchoRequestHandler::new, WireType.BINARY), VanillaNetworkContext::new);
			eg.addHandler(eah);
			LockSupport.park();
		}
