...
/**
 * IMPORTANT - each event handler MUST run in its own thread, as the handshake
 * is a blocking operation. Spinning the work out to a separate thread does not
 * work, as subsequent invocations of the TcpEventHandler may consume socket
...
public final class NonClusteredSslIntegrationTest extends NetworkTestCommon {
	...
	private void doConnect() {
		if (mode == Mode.CLIENT_TO_SERVER || mode == Mode.BI_DIRECTIONAL) {
			try (RemoteConnector<StubNetworkContext> connector = new RemoteConnector<StubNetworkContext>(nc -> {
				final TcpEventHandler<StubNetworkContext> eventHandler = new TcpEventHandler<>(nc);
				eventHandler.tcpHandler(getTcpHandler(clientInitiator));
				return eventHandler;
			})) {
				connector.connect("server", client, new StubNetworkContext(), 1000L);
			}
		}
		...
		if (mode == Mode.SERVER_TO_CLIENT || mode == Mode.BI_DIRECTIONAL) {
			try (RemoteConnector<StubNetworkContext> connector = new RemoteConnector<StubNetworkContext>(nc -> {
				...
				connector.connect("client", server, new StubNetworkContext(), 1000L);
			}
		}
	}
	...
	private enum Mode {
		CLIENT_TO_SERVER, SERVER_TO_CLIENT, BI_DIRECTIONAL
	}

	private static final class CountingTcpHandler extends SimpleCloseable implements TcpHandler<StubNetworkContext> {
				...
				if (nc.isAcceptor() && in.readRemaining() != 0) {
					...
					if (DEBUG) {
						if (len > 10) {
							...
							System.out.printf("%s received [%d] %d/%s%n", label, tmp.length, received,
	...
	private static final class StubNetworkContext extends VanillaNetworkContext<StubNetworkContext>
		...
		public NetworkStatsListener<StubNetworkContext> networkStatsListener() {
			return new NetworkStatsListener<StubNetworkContext>() {
				...
				@Override
				public void onNetworkStats(final long writeBps, final long readBps,
						final long socketPollCountPerSecond) {
