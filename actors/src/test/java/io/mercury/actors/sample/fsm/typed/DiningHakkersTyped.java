...
//
//			return Behaviors.receive(ChopstickMessage.class).onMessage(ChopstickMessage.Take.class, (ctx, msg) -> {
//				msg.hakker.tell(new ChopstickAnswer.Busy(ctx.getSelf()));
//				return Behaviors.same();
//			}).onMessage(ChopstickMessage.Put.class, m -> m.hakker.equals(hakker), (ctx, msg) -> available).build();
//
//		}
//
...
//			return Behaviors.receive(HakkerMessage.class).onMessage(HakkerMessage.Think.class, (ctx, msg) -> {
...
//				ActorRef<ChopstickAnswer> adapter = ctx.messageAdapter(ChopstickAnswer.class,
//						ChopstickAnswerAdaptor::new);
//				return Behaviors.receive(HakkerMessage.class).onMessageEquals(HakkerMessage.Eat.INSTANCE, msg -> {
...
//							(ctx, msg) -> waitForOtherChopstick(right, left))
//					.onMessage(ChopstickAnswerAdaptor.class, m -> m.msg.isTakenBy(right),
//							(ctx, msg) -> waitForOtherChopstick(left, right))
//					.onMessage(ChopstickAnswerAdaptor.class, m -> m.msg.isBusy(), (ctx, msg) -> firstChopstickDenied())
//					.build();
//		}
//
...
//				ActorRef<ChopstickAnswer> adapter = ctx.messageAdapter(ChopstickAnswer.class,
//						ChopstickAnswerAdaptor::new);
//				return Behaviors.receive(HakkerMessage.class).onMessage(ChopstickAnswerAdaptor.class,
//						m -> m.msg.isTakenBy(chopstickToWaitFor), (context, msg) -> {
...
//				ActorRef<ChopstickAnswer> adapter = ctx.messageAdapter(ChopstickAnswer.class,
//						ChopstickAnswerAdaptor::new);
//				return Behaviors.receive(HakkerMessage.class).onMessageEquals(HakkerMessage.Think.INSTANCE, ignore -> {
...
//				ActorRef<ChopstickAnswer> adapter = context.messageAdapter(ChopstickAnswer.class,
//						ChopstickAnswerAdaptor::new);
//				return Behaviors.receive(HakkerMessage.class)
...
//
//			// Create 5 chopsticks
//			List<ActorRef<ChopstickMessage>> chopsticks = new ArrayList<>();
//			for (int i = 0; i < 5; i++) {
...
//			List<ActorRef<HakkerMessage>> hakkers = new ArrayList<>();
//			int i = 0;
