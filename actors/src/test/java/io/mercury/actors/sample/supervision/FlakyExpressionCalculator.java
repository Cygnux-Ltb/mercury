...
public class FlakyExpressionCalculator extends AbstractLoggingActor {
    ...
    public record Result(Expression originalExpression, Integer value, Position position) {
    }
    ...
    public void preStart() {
        if (expr instanceof Const) {
            Integer value = ((Const) expr).value();
    ...
    public Receive createReceive() {
        return receiveBuilder().match(Result.class, result0 -> expected.contains(result0.position()), result1 -> {
            expected.remove(result1.position());
            results.put(result1.position(), result1.value());
            if (results.size() == 2) {
            ...
            throw new IllegalStateException("Expected results for positions "
                    + expected.stream().map(Object::toString).collect(Collectors.joining(", ")) + " but got position "
                    + result.position());
        }).build();
    }
