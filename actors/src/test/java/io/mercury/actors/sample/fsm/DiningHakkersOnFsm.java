...
public class DiningHakkersOnFsm {
     ...
     */
    public record TakenBy(ActorRef hakker) {
    }
    ...
    public static class Chopstick extends AbstractLoggingFSM<CS, TakenBy> {
        {
            ...
            when(CS.Available, matchEventEquals(Take,
                    (take, data) -> goTo(CS.Taken).using(new TakenBy(sender())).replying(new Taken(self()))));
     ...
     */
    public record TakenChopsticks(ActorRef left, ActorRef right) {
    }
    ...
    public static class Hakker extends AbstractLoggingFSM<HS, TakenChopsticks> {
...

        {
            ...
            when(HS.Hungry,
                    matchEvent(Taken.class, (taken, data) -> taken.chopstick == left,
                            ...
                            .event(Taken.class, (taken, data) -> taken.chopstick == right,
                                    (taken, data) -> goTo(HS.WaitForOtherChopstick)
                                            .using(new TakenChopsticks(null, right)))
                            .event(Busy.class, (busy, data) -> goTo(HS.FirstChopstickDenied)));
            ...
            when(HS.WaitForOtherChopstick,
                    matchEvent(Taken.class,
                            ...
                            .event(Taken.class,
                                    (taken, data) -> (taken.chopstick == right && data.left != null
                                            && data.right == null),
                                    (taken, data) -> startEating(left, right))
        ...
        private FSM.State<HS, TakenChopsticks> startEating(ActorRef left, ActorRef right) {
            System.out.printf("%s has picked up %s and %s and starts to eat%n", name, left.path().name(),
                    right.path().name());
