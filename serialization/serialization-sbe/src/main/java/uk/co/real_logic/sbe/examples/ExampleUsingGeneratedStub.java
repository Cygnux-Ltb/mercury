...
public class ExampleUsingGeneratedStub {
	...
	static int encode(final CarEncoder car, final UnsafeBuffer directBuffer,
		...
		// Please use a suitable schema type for varDataEncoding.length: uint8 <= 254,
		// uint16 <= 65534
		car.manufacturer(new String(MANUFACTURER, StandardCharsets.UTF_8)).putModel(MODEL, 0, MODEL.length)
				.putActivationCode(ACTIVATION_CODE, 0, ACTIVATION_CODE.capacity());

		return MessageHeaderEncoder.ENCODED_LENGTH + car.encodedLength();
	}

	static void decode(final CarDecoder car, final UnsafeBuffer directBuffer, final MessageHeaderDecoder headerDecoder)
			throws Exception {
		...
		sb.append("\ncar.activationCode=")
				.append(new String(buffer, 0, tempBufferLength, CarEncoder.activationCodeCharacterEncoding()));
