...
public class FastSerializerGenerator<T> extends FastSerdeBase {
    ...
    /**
     * Enum schema mapping for Avro-1.4 to record schema id and corresponding schema
     * JVar.
     */
    ...
    public FastSerializer<T> generateSerializer() {
        ...
        try {
            ...
            switch (schema.getType()) {
                case RECORD -> processRecord(schema, serializeMethodParam, serializeMethod.body());
                case ARRAY -> processArray(schema, serializeMethodParam, serializeMethod.body());
                case MAP -> processMap(schema, serializeMethodParam, serializeMethod.body());
                default -> throw new FastSerdeGeneratorException("Unsupported input schema type: " + schema.getType());
            }

            serializeMethod.param(codeModel.ref(Encoder.class), ENCODER);
            serializeMethod._throws(codeModel.ref(IOException.class));

            @SuppressWarnings("unchecked") final Class<FastSerializer<T>> clazz = (Class<FastSerializer<T>>) compileClass(className,
    ...
    private void processComplexType(Schema schema, JExpression valueExpr, JBlock body) {
        switch (schema.getType()) {
            case RECORD -> processRecord(schema, valueExpr, body);
            case ARRAY -> processArray(schema, valueExpr, body);
            case UNION -> processUnion(schema, valueExpr, body);
            case MAP -> processMap(schema, valueExpr, body);
            default -> throw new FastSerdeGeneratorException("Not a complex schema type: " + schema.getType());
        }
    }
    ...
    private void processSimpleType(Schema schema, JExpression valueExpression, JBlock body, boolean cast) {
        switch (schema.getType()) {
            case ENUM -> processEnum(schema, valueExpression, body);
            case FIXED -> processFixed(schema, valueExpression, body);
            default -> processPrimitive(schema, valueExpression, body, cast);
        }
    }
    ...
    private void processArray(final Schema arraySchema, JExpression arrayExpr, JBlock body) {
        ...
        ifCodeGen(body, emptyArrayCondition, then1 -> then1.invoke(JExpr.direct(ENCODER), "setItemCount").arg(JExpr.lit(0)), else1 -> {
            ...
            if (SchemaAssistant.isPrimitive(arraySchema.getElementType())) {
                ...
                }, else2 -> processArrayElementLoop(arraySchema, arrayClass, arrayExpr, else2, "get"));
            } else {
    ...
    private void processArrayElementLoop(final Schema arraySchema, final JClass arrayClass, JExpression arrayExpr,
                                         JBlock body, String getMethodName) {
    ...
    private void processMap(final Schema mapSchema, JExpression mapExpr, JBlock body) {
        ...
        if (SchemaAssistant.hasStringableKey(mapSchema)) {
            keyStringVar = forBody.decl(codeModel.ref(String.class), getUniqueName("keyString"),
    ...
    /**
     * Avro-1.4 doesn't provide function: "getIndexNamed", so we just create the
     * following function with the similar logic, which will work with both Avro-1.4
     * and Avro-1.7.
     *
    ...
    private void processUnion(final Schema unionSchema, JExpression unionExpr, JBlock body) {
        ...
        for (Schema schemaOption : unionSchema.getTypes()) {
            if (Schema.Type.NULL.equals(schemaOption.getType())) {
                /*
                 * We always handle the null branch of the union first, otherwise, it leads to a
                 * bug in the case where there is an optional field where the null is the second
                 * branch of the union.
                 */
        ...
        for (Schema schemaOption : unionSchema.getTypes()) {
            if (Schema.Type.NULL.equals(schemaOption.getType())) {
                /*
                 * Since we've already added code to process the null branch, we can skip it
                 * when processing the other types.
                 */
                continue;
            }
            ...
            /*
             * In Avro-1.4, neither GenericEnumSymbol nor GenericFixed has associated schema,
             * so we don't expect to see two or more Enum types or two or more Fixed types
             * in the same Union in generic mode since the writer couldn't differentiate the
             * Enum types or the Fixed types, but those scenarios are well-supported in
             * Avro-1.7 or above since both of them have associated 'Schema', so the
             * serializer could recognize the right type by checking the associated 'Schema'
             * in generic mode.
             */
            if (useGenericTypes && SchemaAssistant.isNamedTypeWithSchema(schemaOption)) {
                condition = unionExpr._instanceof(rawOptionClass).cand(JExpr
                        .invoke(JExpr.lit(AvroCompatibilityHelper.getSchemaFullName(schemaOption)), "equals")
    ...
    private void processEnum(Schema enumSchema, JExpression enumValueExpression, JBlock body) {
        ...
        if (useGenericTypes) {
            if (Utils.isAvro14()) {
                /*
                 * In Avro-1.4, there is no way to infer/extract enum schema from
                 * {@link org.apache.avro.generic.GenericData.EnumSymbol}, so the serializer
                 * needs to record the schema id and the corresponding
                 * {@link org.apache.avro.Schema.EnumSchema}, and maintain a mapping between the
                 * schema id and EnumSchema JVar for future use.
                 */
                JVar enumSchemaVar = enumSchemaVarMap.computeIfAbsent(Utils.getSchemaFingerprint(enumSchema),
                        s -> generatedClass.field(JMod.PRIVATE | JMod.FINAL, Schema.class,
    ...
    private void processString(final Schema primitiveSchema, JExpression primitiveValueExpression, JBlock body) {
        ...
        if (!useGenericTypes && SchemaAssistant.isStringable(primitiveSchema)) {
            ...
            stringTypeCheck._then().invoke(encodeVar, writeFunction)
    ...
    private void processPrimitive(final Schema primitiveSchema, JExpression primitiveValueExpression, JBlock body,
        ...
        JExpression writeFunctionArgument = cast ? JExpr.cast(primitiveClass, primitiveValueExpression)
        ...
        switch (primitiveSchema.getType()) {
            ...
            case BYTES -> writeFunction = "writeBytes";
            case INT -> writeFunction = "writeInt";
            case LONG -> writeFunction = "writeLong";
            case FLOAT -> writeFunction = "writeFloat";
            case DOUBLE -> writeFunction = "writeDouble";
            case BOOLEAN -> writeFunction = "writeBoolean";
            default ->
                    throw new FastSerdeGeneratorException("Unsupported primitive schema of type: " + primitiveSchema.getType());
        }
    ...
    private boolean methodAlreadyDefined(final Schema schema) {
        return !Schema.Type.RECORD.equals(schema.getType())
                || serializeMethodMap.containsKey(AvroCompatibilityHelper.getSchemaFullName(schema));
    }
    ...
    private JMethod getMethod(final Schema schema) {
        if (Schema.Type.RECORD.equals(schema.getType())) {
            ...
            throw new FastSerdeGeneratorException(
                    "No method for schema: " + AvroCompatibilityHelper.getSchemaFullName(schema));
        }
    ...
    private JMethod createMethod(final Schema schema) {
        if (Schema.Type.RECORD.equals(schema.getType())) {
            if (!methodAlreadyDefined(schema)) {
                JMethod method = generatedClass.method(JMod.PUBLIC, codeModel.VOID,
                ...
                throw new FastSerdeGeneratorException(
                        "Method already exists for: " + AvroCompatibilityHelper.getSchemaFullName(schema));
            }
        }
        throw new FastSerdeGeneratorException("No method for schema type: " + schema.getType());
    }
