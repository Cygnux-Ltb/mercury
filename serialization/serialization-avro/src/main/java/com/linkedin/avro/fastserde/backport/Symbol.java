...
public abstract class Symbol {
     ...
     * this is null. Otherwise, this holds the sequence of
    ...
    /**
     * Flattens the given sub-array of symbols into a sub-array of symbols. Every
     ...
     * has not been fully constructed yet, we copy a bunch of nulls.
    ...
    protected static int flattenedSize(Symbol[] symbols, int start) {
        ...
        for (int i = start; i < symbols.length; i++) {
            if (symbols[i] instanceof Sequence s) {
    ...
    public static boolean hasErrors(Symbol symbol) {
        switch (symbol.kind) {
            ...
            default -> throw new RuntimeException("unknown symbol kind: " + symbol.kind);
        }
    }
