...
public class FastDeserializerGenerator<T> extends FastDeserializerGeneratorBase<T> {
    ...
    /**
     * This is sometimes passed into the reuse parameter, and Avro treats null as a
     * sentinel value indicating that it should instantiate a new object instead of
     * re-using.
     */
    private static final Supplier<JExpression> EMPTY_SUPPLIER = JExpr::_null;

    private JMethod constructor;
    private final Map<Long, Schema> schemaMap = new HashMap<>();
    private final Map<Long, JVar> schemaVarMap = new HashMap<>();
    ...
    public FastDeserializer<T> generateDeserializer() {
        ...
        try {
            ...
            switch (aliasedWriterSchema.getType()) {
                ...
                case ARRAY ->
                        processArray(readerSchemaVar, "array", aliasedWriterSchema, reader, topLevelDeserializeBlock,
                                fieldAction, JBlock::_return, reuseSupplier);
                case MAP ->
                ...
                default -> throw new FastDeserializerGeneratorException(
            ...
            Class<FastDeserializer<T>> clazz = (Class<FastDeserializer<T>>) compileClass(className,
                    schemaAssistant.getUsedFullyQualifiedClassNameSet());
        ...
        switch (schema.getType()) {
            ...
            case UNION ->
                    processUnion(fieldSchemaVar, name, schema, readerFieldSchema, methodBody, action, putExpressionIntoParent,
                            reuseSupplier);
            default -> throw new FastDeserializerGeneratorException("Incorrect complex type: " + action.getType());
        }
    }
    ...
    private void processSimpleType(Schema schema, Schema readerSchema, JBlock methodBody, FieldAction action,
        ...
        switch (schema.getType()) {
            case ENUM -> processEnum(readerSchema, methodBody, action, putExpressionIntoParent);
            case FIXED -> processFixed(schema, methodBody, action, putExpressionIntoParent, reuseSupplier);
            default -> {
    ...
    private void processRecord(JVar recordSchemaVar, String recordName, final Schema recordWriterSchema,
        ...
        if (recordAction.getShouldRead()) {
            putRecordIntoParent.accept(parentBody,
                    JExpr.invoke(method).arg(reuseSupplier.get()).arg(JExpr.direct(DECODER)));
        } else {
        ...
        if (recordAction.getShouldRead()) {
            ...
            if (useGenericTypes) {
                ...
                newRecord.arg(recordSchema);

                ifCodeGen(methodBody,
                        // Check whether schema matches
                        reuseVar.ne(JExpr._null()).cand(reuseVar._instanceof(indexedRecordClass)).cand(
                                // The following statement will produce the equality check by "==", so that the
                                // comparison is
                                // not trying to compare the schema content, but just the reference, which
                                // should be very fast.
                        ...
                        elseBlock -> elseBlock.assign(result, newRecord));
            } else {
        ...
        for (Schema.Field field : recordWriterSchema.getFields()) {
            // We roll the population method for very large records, the initial fields are
            // kept in the outer method as original to maintain performance for smaller
            // records
            fieldCount++;
            ...
            if (SchemaAssistant.isComplexType(field.schema())) {
                processComplexType(fieldSchemaVar, field.name(), field.schema(), readerFieldSchema, popMethodBody,
                        action, putExpressionInRecord, fieldReuseSupplier);
            } else {
                processSimpleType(field.schema(), readerFieldSchema, popMethodBody, action, putExpressionInRecord,
                        fieldReuseSupplier);
            }
        ...
        if (recordAction.getShouldRead()) {
            ...
            for (Schema.Field readerField : recordReaderSchema.getFields()) {
                if (!fieldNamesSet.contains(readerField.name())) {
                    ...
                    JExpression value = parseDefaultValue(readerField.schema(),
                            AvroCompatibilityHelper.getGenericDefaultValue(readerField), methodBody, schemaVar,
                            readerField.name());
    ...
    private JExpression parseDefaultValue(Schema schema, Object defaultValue, JBlock body, JVar schemaVar,
        ...
        if (SchemaAssistant.isComplexType(schema)) {
            ...
            switch (schemaType) {
                case RECORD -> {
                    if (useGenericTypes) {
                        valueInitializationExpr.arg(getSchemaExpr(schema));
                    }
                    ...
                    for (Schema.Field subField : schema.getFields()) {
                        ...
                        JExpression fieldValue = parseDefaultValue(subField.schema(),
                                defaultValueRecord.get(subField.name()), body, fieldSchemaVar, subField.name());
                ...
                case ARRAY -> {
                    ...
                    if (useGenericTypes) {
                        valueInitializationExpr.arg(JExpr.lit(defaultValueList.size()))
                                .arg(getSchemaExpr(schema));
                ...
                case MAP -> {
                    ...
                    for (Map.Entry<CharSequence, Object> mapEntry : defaultValueMap.entrySet()) {
                        ...
                        if (SchemaAssistant.hasStringableKey(schema)) {
                            mapKeyExpr = JExpr._new(schemaAssistant.findStringClass(schema))
                                    .arg(mapEntry.getKey().toString());
                        } else {
                        ...
                        JExpression mapEntryValueExpression = parseDefaultValue(schema.getValueType(), mapEntry.getValue(),
                                body, mapValueSchemaVar, "mapElement");
                        body.invoke(valueVar, "put").arg(mapKeyExpr).arg(mapEntryValueExpression);
                    }
                }
                default -> throw new FastDeserializerGeneratorException("Incorrect schema type in default value!");
            }
            return valueVar;
        } else {
            switch (schemaType) {
                ...
                case FIXED:
                    ...
                    // For specific Fixed type, Avro-1.4 will only generate the class with default
    ...
    private void processUnion(JVar unionSchemaVar, final String name, final Schema unionSchema,
        ...
        for (int i = 0; i < unionSchema.getTypes().size(); i++) {
            ...
            if (action.getShouldRead()) {
                // The reader's union could be re-ordered, so we need to find the one that
                // matches.
                for (int j = 0; j < readerUnionSchema.getTypes().size(); j++) {
                    Schema potentialReaderSchema = readerUnionSchema.getTypes().get(j);
                    // Avro allows unnamed types to appear only once in a union, but named types may
                    ...
                    if (potentialReaderSchema.getType().equals(optionSchema.getType())
                            ...
                            || AvroCompatibilityHelper.getSchemaFullName(potentialReaderSchema)
                            .equals(AvroCompatibilityHelper.getSchemaFullName(optionSchema))
                            || potentialReaderSchema.getAliases()
                            .contains(AvroCompatibilityHelper.getSchemaFullName(optionSchema)))) {
                ...
                if (null == readerOptionSchema) {
                    ...
                    thenBlock._throw(JExpr._new(codeModel.ref(AvroTypeException.class)).arg(JExpr
                            .lit("Found " + optionSchema + ", expecting " + readerUnionSchema.getTypes().toString())));
                    continue;
                }
            ...
            if (useGenericTypes && unionAction.getShouldRead()) {
                if (-1 == readerOptionUnionBranchIndex) {
                    // TODO: Improve the flow of this code... it's messy needing to check for this
                    // here...
                    throw new FastSerdeGeneratorException(
                            "Unexpected internal state. The readerOptionUnionBranchIndex should be defined if unionAction.getShouldRead() == true.");
                }
                JInvocation optionSchemaExpression = unionSchemaVar.invoke("getTypes").invoke("get")
                        .arg(JExpr.lit(readerOptionUnionBranchIndex));
            ...
            if (SchemaAssistant.isComplexType(optionSchema)) {
                ...
                processComplexType(optionSchemaVar, optionName, optionSchema, readerOptionSchema, thenBlock,
                        unionAction, putValueIntoParent, reuseSupplier);
            } else {
                processSimpleType(optionSchema, readerOptionSchema, thenBlock, unionAction, putValueIntoParent,
                        reuseSupplier);
            }
        }
        if (ifBlock != null) {
            ifBlock._else()._throw(JExpr._new(codeModel.ref(RuntimeException.class))
                    .arg(JExpr.lit("Illegal union index for '" + name + "': ").plus(unionIndex)));
        }
    }
    ...
    private void processArray(JVar arraySchemaVar, final String name, final Schema arraySchema,
        ...
        final JVar arrayVar = action.getShouldRead()
                ? declareValueVar(name, readerArraySchema, parentBody, true, false, true)
                : null;
        /*
         * Special optimization for float array by leveraging
         * {@link ByteBufferBackedPrimitiveFloatList}.
         *
        ...
        if (action.getShouldRead() && arraySchema.getElementType().getType().equals(Schema.Type.FLOAT)) {
            ...
            JExpression castedResult = JExpr.cast(codeModel.ref(PrimitiveFloatList.class),
                    readPrimitiveFloatArrayInvocation);
        ...
        final Supplier<JExpression> finalReuseSupplier = potentiallyCacheInvocation(reuseSupplier, parentBody,
                "oldArray");
        if (finalAction.getShouldRead()) {

            JClass arrayClass = schemaAssistant.classFromSchema(readerArraySchema, false, false, true);
            JClass abstractErasedArrayClass = schemaAssistant.classFromSchema(readerArraySchema, true, false, true)
                    .erasure();
            ...
            if (useGenericTypes && !SchemaAssistant.isPrimitive(arraySchema.getElementType())) {
                /*
                 * N.B.: The ColdPrimitiveXList implementations do not take the schema as a
                 * constructor param, but the {@link org.apache.avro.generic.GenericData.Array}
                 * does.
                 */
            ...
            /*
             * N.B.: Need to use the erasure because instanceof does not support generic
             * types
             */
        ...
        JVar counter = forLoopToIterateOnElementsOfCurrentBlock.init(codeModel.INT, getUniqueName("counter"),
                JExpr.lit(0));
        ...
        if (finalAction.getShouldRead()) {
            String addMethod = SchemaAssistant.isPrimitive(arraySchema.getElementType()) ? "addPrimitive" : "add";
        ...
        if (SchemaAssistant.isCapableOfReuse(arraySchema.getElementType())) {
            ...
            JVar elementReuseVar = forBody.decl(codeModel.ref(Object.class),
                    getUniqueName(name + "ArrayElementReuseVar"), JExpr._null());
            ifCodeGen(forBody, finalReuseSupplier.get()._instanceof(codeModel.ref(GenericArray.class)), then2 -> {
                then2.assign(elementReuseVar,
                        JExpr.invoke(JExpr.cast(codeModel.ref(GenericArray.class), finalReuseSupplier.get()), "peek"));
            });
        ...
        if (SchemaAssistant.isComplexType(arraySchema.getElementType())) {
            ...
            processComplexType(elementSchemaVar, elemName, arraySchema.getElementType(), readerArrayElementSchema,
                    forBody, finalAction, putValueInArray, elementReuseSupplier);
        } else {
            processSimpleType(arraySchema.getElementType(), readerArrayElementSchema, forBody, finalAction,
                    putValueInArray, elementReuseSupplier);
        }
    ...
    /**
     * Return a JExpression, which will read a string from decoder and construct a
     * stringable object.
     *
     * @param stringableClass JClass
     * @return JExpression
     */
    private JExpression readStringableExpression(JClass stringableClass) {
        ...
        return JExpr._new(stringableClass).arg(stringableArgExpr);
    }
    ...
    private void processMap(JVar mapSchemaVar, final String name, final Schema mapSchema, final Schema readerMapSchema,
        ...
        /*
         * Determine the action symbol for Map value. {@link ResolvingGrammarGenerator}
         * generates resolving grammar symbols with reversed order of production
         * sequence. If this symbol is a terminal, its production list will be
         * <code>null</code>. Otherwise the production list holds the the sequence of
         * the symbols that forms this symbol.
         *
         * The {@link FastDeserializerGenerator.generateDeserializer} tries to proceed
         * as a depth-first, left-to-right traversal of the schema. So for a nested Map,
         * we need to iterate production list in reverse order to get the correct
         * "map-end" symbol of internal Maps.
         */
        if (action.getShouldRead()) {
            Symbol valuesActionSymbol = null;
        ...
        if (action.getShouldRead()) {
            ...
            final Supplier<JExpression> finalReuseSupplier = potentiallyCacheInvocation(reuseSupplier,
                    ifBlockForChunkLenCheck, "oldMap");
            ifCodeGen(ifBlockForChunkLenCheck, finalReuseSupplier.get()._instanceof(codeModel.ref(Map.class)),
                    thenBlock -> thenBlock.assign(reuse,
                            JExpr.cast(codeModel.ref(Map.class), finalReuseSupplier.get())));
            ...
            ifCodeGen(ifBlockForChunkLenCheck, reuse.ne(JExpr.direct("null")), thenBlock -> {
                    ...
                    // The default load factor of HashMap is 0.75 and HashMap internally ensures
                    // size is always a power of two.
                    elseBlock -> elseBlock.assign(mapVar,
                            JExpr._new(schemaAssistant.classFromSchema(readerMapSchema, false)).arg(JExpr.cast(
                                    codeModel.INT, chunkLen.mul(JExpr.lit(4)).plus(JExpr.lit(2)).div(JExpr.lit(3))))));

            JBlock elseBlock = conditional._else();
            elseBlock.assign(mapVar,
                    JExpr._new(schemaAssistant.classFromSchema(readerMapSchema, false)).arg(JExpr.lit(0)));
        }
        ...
        if (SchemaAssistant.hasStringableKey(mapSchema)) {
            ...
            keyValueExpression = codeModel.ref(String.class).equals(keyClass) ? JExpr.direct(DECODER + ".readString()")
        ...
        if (action.getShouldRead() && useGenericTypes) {
            mapValueSchemaVar = declareSchemaVar(readerMapSchema.getValueType(), name + "MapValueSchema",
        ...
        if (SchemaAssistant.isComplexType(mapSchema.getValueType())) {
            ...
            processComplexType(mapValueSchemaVar, valueName, mapSchema.getValueType(), readerMapValueSchema, forBody,
                    action, putValueInMap, EMPTY_SUPPLIER);
        } else {
            processSimpleType(mapSchema.getValueType(), readerMapValueSchema, forBody, action, putValueInMap,
                    EMPTY_SUPPLIER);
        }
    ...
    private void processFixed(final Schema schema, JBlock body, FieldAction action,
        ...
        if (action.getShouldRead()) {
            ...
            if (reuseSupplier.get().equals(JExpr._null())) {
                ...
                /*
                 * Here will check whether the length of the reused fixed is same as the one to
                 * be deserialized or not. If not, here will initialize a new byte array to
                 * store it.
                 */
                ...
                ifCodeGen(body,
                        oldValue._instanceof(codeModel.ref(GenericFixed.class))
                                .cand(JExpr.invoke(JExpr.cast(codeModel.ref(GenericFixed.class), oldValue), "bytes")
                                        .ref("length").eq(JExpr.direct("" + schema.getFixedSize()))),
                        thenBlock -> thenBlock.assign(fixedBuffer,
                                JExpr.invoke(JExpr.cast(codeModel.ref(GenericFixed.class), oldValue), "bytes")),
                        elseBlock -> elseBlock.assign(fixedBuffer,
                                JExpr.direct(" new byte[" + schema.getFixedSize() + "]")));
            }
    ...
    private void processEnum(final Schema schema, final JBlock body, FieldAction action,
        ...
        if (action.getShouldRead()) {
            ...
            if (enumOrderCorrect) {
                ...
                /*
                 * Define a class variable to keep the mapping between the enum index from the
                 * writer schema and the corresponding one in the reader schema, and there are
                 * some cases: 1. If the enum index doesn't exist in this map, runtime will
                 * throw RuntimeException. 2. If the enum index exists in the map, runtime will
                 * throw it if the mapping is an {@link AvroTypeException} because of unknown
                 * enum from reader schema. 3. If there is a corresponding enum in the reader
                 * schema, runtime will use this one to lookup the exact Enum Symbol from the
                 * reader schema.
                 */
                JVar tempEnumMappingVar = constructor.body().decl(codeModel.ref(HashMap.class),
                        getUniqueName("tempEnumMapping"),
                ...
                for (int i = 0; i < enumAdjustAction.adjustments.length; i++) {
                    ...
                    if (unknownEnumIndexes.contains(i)) {
                        JInvocation avroTypeExceptionExpr = JExpr._new(codeModel.ref(AvroTypeException.class)).arg(
                                JExpr.lit(schema.getFullName() + ": " + enumAdjustAction.adjustments[i].toString()));
                        mapUpdateStatement = tempEnumMappingVar.invoke("put").arg(keyExpr).arg(avroTypeExceptionExpr);
                    } else {
                        JInvocation valueExpr = JExpr._new(codeModel.ref(Integer.class))
                ...
                JVar enumMappingVar = generatedClass.field(JMod.PRIVATE | JMod.FINAL, Map.class,
                        getUniqueName("enumMapping" + schema.getName()));
                constructor.body().assign(JExpr.refthis(enumMappingVar.name()),
                        codeModel.ref(Collections.class).staticInvoke("unmodifiableMap").arg(tempEnumMappingVar));
                ...
                JExpression ithValResult = schemaAssistant.getEnumValueByIndex(schema,
                        JExpr.cast(codeModel.ref(Integer.class), lookupResult), getSchemaExpr(schema));
                ifBlock._then().assign((JVar) newEnum, ithValResult);
                /*
                 * Unknown enum in reader schema.
                 */
                JConditional elseIfBlock = ifBlock
                        ._elseif(lookupResult._instanceof(codeModel.ref(AvroTypeException.class)));
    ...
    private void processBytes(JBlock body, FieldAction action, BiConsumer<JBlock, JExpression> putValueIntoParent,
        ...
        if (action.getShouldRead()) {
            if (reuseSupplier.get().equals(JExpr._null())) {
                putValueIntoParent.accept(body,
                        JExpr.invoke(JExpr.direct(DECODER), "readBytes").arg(JExpr.direct("null")));
            } else {
                final Supplier<JExpression> finalReuseSupplier = potentiallyCacheInvocation(reuseSupplier, body,
                        "oldBytes");
                ifCodeGen(body, finalReuseSupplier.get()._instanceof(codeModel.ref("java.nio.ByteBuffer")),
                        thenBlock -> putValueIntoParent.accept(thenBlock,
                                JExpr.invoke(JExpr.direct(DECODER), "readBytes")
            ...
            if (stringClass.equals(codeModel.ref(Utf8.class))) {
                if (reuseSupplier.equals(EMPTY_SUPPLIER)) {
                    ...
                    ifCodeGen(body, finalReuseSupplier.get()._instanceof(codeModel.ref(Utf8.class)),
                            thenBlock -> putValueIntoParent.accept(thenBlock,
                                    JExpr.invoke(JExpr.direct(DECODER), "readString")
                                            .arg(JExpr.cast(codeModel.ref(Utf8.class), finalReuseSupplier.get()))),
                            elseBlock -> putValueIntoParent.accept(elseBlock,
                                    JExpr.invoke(JExpr.direct(DECODER), "readString").arg(JExpr._null())));
                }
    ...
    private void processPrimitive(final Schema schema, JBlock body, FieldAction action,
        ...
        switch (schema.getType()) {
            ...
            case INT -> readFunction = "readInt()";
            case LONG -> readFunction = "readLong()";
            case FLOAT -> readFunction = "readFloat()";
            case DOUBLE -> readFunction = "readDouble()";
            case BOOLEAN -> readFunction = "readBoolean()";
            default ->
    ...
    private JVar declareSchemaVar(Schema valueSchema, String variableName, JInvocation getValueType) {
        ...
        /*
         * TODO: In theory, we should only need Record, Enum and Fixed here since only
         * these types require schema for the corresponding object initialization in
         * Generic mode.
         */
    ...
    private boolean methodAlreadyDefined(final Schema writerSchema, final Schema readerSchema, boolean read) {
        if (!Schema.Type.RECORD.equals(writerSchema.getType())) {
            throw new FastDeserializerGeneratorException(
                    "Methods are defined only for records, not for " + writerSchema.getType());
        }

        return (read ? deserializeMethodMap : skipMethodMap)
                .containsKey(getEffectiveMethodName(writerSchema, readerSchema));
    }
    ...
    private JMethod getMethod(final Schema writerSchema, final Schema readerSchema, boolean read) {
        ...
        if (!methodAlreadyDefined(writerSchema, readerSchema, read)) {
            throw new FastDeserializerGeneratorException(
                    "No method for writerSchema: " + AvroCompatibilityHelper.getSchemaFullName(writerSchema));
        }
    ...
    private JMethod createMethod(final Schema writerSchema, final Schema readerSchema, boolean read) {
        ...
        if (methodAlreadyDefined(writerSchema, readerSchema, read)) {
            throw new FastDeserializerGeneratorException(
                    "Method already exists for: " + AvroCompatibilityHelper.getSchemaFullName(writerSchema));
        }
    ...
    private Supplier<JExpression> potentiallyCacheInvocation(Supplier<JExpression> jExpressionSupplier, JBlock body,
                                                             String variableNamePrefix) {
        if (jExpressionSupplier.get() instanceof JInvocation) {
            JVar oldValue = body.decl(codeModel.ref(Object.class), getUniqueName(variableNamePrefix),
                    jExpressionSupplier.get());
            return () -> oldValue;
        }
