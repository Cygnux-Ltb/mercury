...
/**
 * Fast avro serializer/deserializer cache. Stores generated and already
 * compiled instances of serializers and deserializers for future use.
 */
...
public final class FastSerdeCache {
    ...
    private final Map<String, FastDeserializer<?>> fastSpecificRecordDeserializersCache = new FastAvroConcurrentHashMap<>();
    private final Map<String, FastDeserializer<?>> fastGenericRecordDeserializersCache = new FastAvroConcurrentHashMap<>();

    private final Map<String, FastSerializer<?>> fastSpecificRecordSerializersCache = new FastAvroConcurrentHashMap<>();
    private final Map<String, FastSerializer<?>> fastGenericRecordSerializersCache = new FastAvroConcurrentHashMap<>();
    ...
    /**
     * @param compileClassPathSupplier custom classpath {@link Supplier}
     */
    ...
    /**
     * @param executorService          {@link Executor} used by
     *                                 serializer/deserializer compile threads
     * @param compileClassPathSupplier custom classpath {@link Supplier}
     */
    ...
    /**
     * @param executorService  customized {@link Executor} used by
     *                         serializer/deserializer compile threads
     * @param compileClassPath custom classpath as string
     */
    ...
    /**
     * Gets default {@link FastSerdeCache} instance. Default instance classpath can
     * be customized via {@value #CLASSPATH} or {@value #CLASSPATH_SUPPLIER} system
     * properties.
     *
    ...
    public static FastSerdeCache getDefaultInstance() {
        if (_INSTANCE == null) {
            synchronized (FastSerdeCache.class) {
                if (_INSTANCE == null) {
                    StringBuilder classPath = new StringBuilder(System.getProperty(CLASSPATH));
                    ...
                    if (classpathSupplierClassName != null) {
                        ...
                        try {
                            ...
                            if (Supplier.class.isAssignableFrom(classPathSupplierClass) && String.class
                                    .equals(((ParameterizedType) classPathSupplierClass.getGenericSuperclass())
                        ...
                        _INSTANCE = new FastSerdeCache(classPath.toString());
                    } else {
                        ...
                        try {
                            ...
                            String avroLibLocation = avroSchemaClass.getProtectionDomain().getCodeSource().getLocation()
                                    .getFile();
                            // find all other libs
                            File avroLibFile = new File(avroLibLocation).getParentFile();
                            for (File lib : Objects.requireNonNull(avroLibFile.listFiles())) {
                                if (lib.getName().endsWith(".jar")) {
                                    classPath.append(":").append(lib.getAbsolutePath());
                                }
                            }
                            LOGGER.debug("Inferred class path: {}", classPath.toString());
    ...
    public static boolean isSupportedForFastSerializer(Schema.Type schemaType) {
        return schemaType.equals(Schema.Type.RECORD) || schemaType.equals(Schema.Type.MAP)
    ...
    /**
     * Generates if needed and returns specific-class aware avro
     * {@link FastDeserializer}.
     *
    ...
    public FastDeserializer<?> getFastSpecificDeserializer(Schema writerSchema, Schema readerSchema) {
        ...
        if (deserializer == null) {
            ...
            deserializer = fastSpecificRecordDeserializersCache.computeIfAbsent(schemaKey, k -> {
                status.set(true);
    ...
    /**
     * Generates if needed and returns generic-class aware avro
     * {@link FastDeserializer}.
     *
     * @param writerSchema {@link Schema} of written data
     * @param readerSchema {@link Schema} intended to be used during deserialization
    ...
    public FastDeserializer<?> getFastGenericDeserializer(Schema writerSchema, Schema readerSchema) {
        ...
        if (deserializer == null) {
            AtomicBoolean status = new AtomicBoolean(false);
            deserializer = fastGenericRecordDeserializersCache.computeIfAbsent(schemaKey, k -> {
                status.set(true);
    ...
    public FastSerializer<?> getFastSpecificSerializer(Schema schema) {
        ...
        if (serializer == null) {
            AtomicBoolean status = new AtomicBoolean(false);
            serializer = fastSpecificRecordSerializersCache.computeIfAbsent(schemaKey, k -> {
                status.set(true);
    ...
    public FastSerializer<?> getFastGenericSerializer(Schema schema) {
        ...
        if (serializer == null) {
            AtomicBoolean status = new AtomicBoolean(false);
            serializer = fastGenericRecordSerializersCache.computeIfAbsent(schemaKey, k -> {
                status.set(true);
    ...
    /**
     * Asynchronously generates if needed and returns specific-class aware avro
     * {@link FastDeserializer}.
     *
     * @param writerSchema {@link Schema} of written data
     * @param readerSchema {@link Schema} intended to be used during deserialization
     * @return {@link CompletableFuture} which contains specific-class aware avro
     * {@link FastDeserializer}
     */
    ...
    /**
     * Asynchronously generates if needed and returns generic-class aware avro
     * {@link FastDeserializer}.
     *
     * @param writerSchema {@link Schema} of written data
     * @param readerSchema {@link Schema} intended to be used during deserialization
     * @return {@link CompletableFuture} which contains generic-class aware avro
     * {@link FastDeserializer}
     */
    public CompletableFuture<FastDeserializer<?>> getFastGenericDeserializerAsync(Schema writerSchema,
                                                                                  Schema readerSchema) {
    ...
    private CompletableFuture<FastDeserializer<?>> getFastDeserializerAsync(Schema writerSchema, Schema readerSchema,
                                                                            Map<String, FastDeserializer<?>> fastDeserializerCache,
                                                                            Supplier<FastDeserializer<?>> fastDeserializerSupplier) {
        String schemaKey = getSchemaKey(writerSchema, readerSchema);
        FastDeserializer<?> deserializer = fastDeserializerCache.get(schemaKey);
        return deserializer != null && isFastDeserializer(deserializer)
                ? CompletableFuture.completedFuture(deserializer)
                : CompletableFuture.supplyAsync(fastDeserializerSupplier, executor).thenApply(d -> {
    ...
    private static String getSchemaKey(Schema writerSchema, Schema readerSchema) {
        return String.valueOf(Math.abs(getSchemaFingerprint(writerSchema)))
                + Math.abs(getSchemaFingerprint(readerSchema));
    }

    /**
     * This function will generate a fast specific deserializer, and it will throw
     * exception if anything wrong happens. This function can be used to verify
     * whether current {@link FastSerdeCache} could generate proper fast
     * deserializer.
     *
    ...
    public FastDeserializer<?> buildFastSpecificDeserializer(Schema writerSchema, Schema readerSchema) {
        FastSpecificDeserializerGenerator<?> generator = new FastSpecificDeserializerGenerator<>(writerSchema,
                readerSchema, classesDir, classLoader, compileClassPath.orElse(null));
        FastDeserializer<?> fastDeserializer = generator.generateDeserializer();

        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(
                    """
                            Generated classes dir: {} and generation of specific FastDeserializer is done for writer schema of type: {} with fingerprint: {} and content: [
                            {}
                            ] and reader schema of type: {} with fingerprint: {} and content: [
                            {}
                            ]""",
                    classesDir, getSchemaFullName(writerSchema), writerSchema.toString(true),
                    getSchemaFingerprint(writerSchema), getSchemaFullName(readerSchema),
                    getSchemaFingerprint(readerSchema), readerSchema.toString(true));
        } else {
            LOGGER.info(
                    "Generated classes dir: {} and generation of specific FastDeserializer is done for writer schema of type: {} with fingerprint: {}"
                            + " and reader schema of type: {} with fingerprint: {}",
                    classesDir, getSchemaFullName(writerSchema), getSchemaFingerprint(writerSchema),
    ...
    /**
     * This function is used to generate a fast generic deserializer, and it will
     * fail back to use {@link SpecificDatumReader} if anything wrong happens.
     *
     * @param writerSchema Schema
    ...
    private FastDeserializer<?> buildSpecificDeserializer(Schema writerSchema, Schema readerSchema) {
        try {
            ...
            LOGGER.warn(
                    """
                            Deserializer generation exception when generating specific FastDeserializer for writer schema: [
                            {}
                            ] and reader schema: [
                            {}
                            ]""",
        ...
        return new FastDeserializer<>() {
            ...
            private final DatumReader datumReader = new SpecificDatumReader<>(writerSchema, readerSchema);
    ...
    /**
     * This function will generate a fast generic deserializer, and it will throw
     * exception if anything wrong happens. This function can be used to verify
     * whether current {@link FastSerdeCache} could generate proper fast
     * deserializer.
     *
    ...
    public FastDeserializer<?> buildFastGenericDeserializer(Schema writerSchema, Schema readerSchema) {
        FastGenericDeserializerGenerator<?> generator = new FastGenericDeserializerGenerator<>(writerSchema,
                readerSchema, classesDir, classLoader, compileClassPath.orElse(null));
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(
                    """
                            Generated classes dir: {} and generation of generic FastDeserializer is done for writer schema of type: {} with fingerprint: {} and content: [
                            {}
                            ] and reader schema of type: {} with fingerprint: {} and content: [
                            {}
                            ]""",
            ...
            LOGGER.info(
                    "Generated classes dir: {} and generation of generic FastDeserializer is done for writer schema of type: {} with fingerprint: {}"
                            + " and reader schema of type: {} with fingerprint: {}",
                    classesDir, getSchemaFullName(writerSchema), getSchemaFingerprint(writerSchema),
                    getSchemaFullName(readerSchema), getSchemaFingerprint(readerSchema));
        }
    ...
    /**
     * This function is used to generate a fast generic deserializer, and it will
     * fail back to use {@link GenericDatumReader} if anything wrong happens.
     *
     * @param writerSchema Schema
     * @param readerSchema Schema
     * @return FastDeserializer<?>
    ...
    private FastDeserializer<?> buildGenericDeserializer(Schema writerSchema, Schema readerSchema) {
        try {
            ...
            LOGGER.warn(
                    "Deserializer generation exception when generating generic FastDeserializer for writer schema: [\n"
        ...
        return new FastDeserializer<>() {
            @SuppressWarnings("rawtypes")
            private final DatumReader datumReader = new GenericDatumReader<>(writerSchema, readerSchema);
    ...
    public FastSerializer<?> buildFastSpecificSerializer(Schema schema) {
        ...
        if (!Utils.isSupportedAvroVersionsForSerializer()) {
            throw new FastDeserializerGeneratorException(
                    "Specific FastSerializer is only supported in following Avro versions: "
                            + Utils.getAvroVersionsSupportedForSerializer());
        }
        FastSpecificSerializerGenerator<?> generator = new FastSpecificSerializerGenerator<>(schema, classesDir,
                classLoader, compileClassPath.orElse(null));

        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(
                    """
                            Generated classes dir: {} and generation of specific FastSerializer is done for schema of type: {} and fingerprint: {} and content: [
                            {}
                            ]""",
                    classesDir, getSchemaFullName(schema), getSchemaFingerprint(schema), schema.toString(true));
        } else {
            LOGGER.info(
                    "Generated classes dir: {} and generation of specific FastSerializer is done for schema of type: {}"
                            + " and fingerprint: {}",
    ...
    private FastSerializer<?> buildSpecificSerializer(Schema schema) {
        if (Utils.isSupportedAvroVersionsForSerializer()) {
            ...
            try {
                ...
                LOGGER.warn(
                        "Serializer generation exception when generating specific FastSerializer for schema: [\n{}\n]",
    ...
    public FastSerializer<?> buildFastGenericSerializer(Schema schema) {
        ...
        if (!Utils.isSupportedAvroVersionsForSerializer()) {
            throw new FastDeserializerGeneratorException(
                    "Generic FastSerializer is only supported in following avro versions:"
                            + Utils.getAvroVersionsSupportedForSerializer());
        }
        FastGenericSerializerGenerator<?> generator = new FastGenericSerializerGenerator<>(schema, classesDir,
                classLoader, compileClassPath.orElse(null));

        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(
                    """
                            Generated classes dir: {} and generation of generic FastSerializer is done for schema of type: {} and fingerprint: {} and content: [
                            {}
                            ]""",
                    classesDir, getSchemaFullName(schema), getSchemaFingerprint(schema), schema.toString(true));
        } else {
            LOGGER.info(
                    "Generated classes dir: {} and generation of generic FastSerializer is done for schema of type: {}"
                            + " and fingerprint: {}",
                    classesDir, getSchemaFullName(schema), getSchemaFingerprint(schema));
        }
    ...
    private FastSerializer<?> buildGenericSerializer(Schema schema) {
        if (Utils.isSupportedAvroVersionsForSerializer()) {
            ...
            try {
                ...
                LOGGER.warn(
                        "Serializer generation exception when generating generic FastSerializer for schema: [\n{}\n]",
                        schema.toString(true), e);
            } catch (Exception e) {
                LOGGER.warn("Serializer class instantiation exception", e);
            }
