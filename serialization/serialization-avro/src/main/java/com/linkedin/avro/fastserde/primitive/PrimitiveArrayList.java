...
public abstract class PrimitiveArrayList<T, L, A> extends AbstractList<T>
	...
	/**
	 * @return the size of the primitive array maintained by the child class, which
	 *         could be larger than {@link #size}.
	 */
	protected abstract int capacity();

	/**
	 * @param capacity of the new primitive array
	 * @return an instance of the right type of primitive array used by the child
	 *         class
	 */
	 ...
	 * @return the comparison result between element of this and that list at the
	 *         provided index
	 */
	 ...
	 * @return true if the passed in object is an instance of the right type of
	 *         primitive list for comparison
	 */
	...
	/**
	 * A function to check if a requested index is within bound of the list. This is
	 * needed because {@link #size} may be smaller than {@link #capacity()}. This
	 * can happen in the case of object reuse, where the previous instance had a
	 * larger number of elements than the new one, in which case, we will reuse the
	 * array, but use the {@link #size} as a boundary within the larger array.
	 *
	 * @param index an index position which may or may not be within bound of the
	 *              current list
	 * @throws IndexOutOfBoundsException if the index param is larger than the index
	 *                                   of the last valid element
	 */
	...
	/**
	 * Checks if the primitve array is at capacity, and if so, resizes it to 1.5x +
	 * 1.
	 */
	...
	/**
	 * Create an empty space in the array at the index specified by shifting the
