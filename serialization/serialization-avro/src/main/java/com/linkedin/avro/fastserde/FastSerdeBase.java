...
import static com.linkedin.avro.fastserde.Utils.generateSourcePathFromPackageName;
...
public abstract class FastSerdeBase {
    ...
    /**
     * A repository of how many times a given name was used. N.B.: Does not actually
     * need to be threadsafe, but it is made so just for defensive coding reasons.
     */
    ...
    public FastSerdeBase(String description, boolean useGenericTypes, Class<?> defaultStringClass, File destination,
                         ClassLoader classLoader, String compileClassPath, boolean isForSerializer) {
    ...
    /**
     * A function to generate unique names, such as those of variables and
     * functions, within the scope of the class instance (i.e. per serializer
     * of a given schema or deserializer of a given schema pair).
     *
    ...
    protected JVar declareValueVar(final String name, final Schema schema, JBlock block, boolean abstractType,
                                   boolean rawType, boolean primitiveList) {
        if (SchemaAssistant.isComplexType(schema)) {
            return block.decl(schemaAssistant.classFromSchema(schema, abstractType, rawType, primitiveList),
                    getUniqueName(name), JExpr._null());
        } else {
    ...
    protected Class<?> compileClass(final String className, Set<String> knownUsedFullyQualifiedClassNameSet)
        ...
        if (null == compiler) {
            /*
             * If the above function returns null, it is very likely that the env setting:
             * "JAVA_HOME" is not being setup properly.
             */
            throw new FastSerdeGeneratorException(
                    "Couldn't locate java compiler at runtime, please double check your env "
                            + "setting for 'JAVA_HOME', and here is the value for 'System.getProperty(\"java.home\")': "
                            + System.getProperty("java.home"));
        }
        String compileClassPathForCurrentFile = Utils.inferCompileDependencies(compileClassPath, filePath,
                knownUsedFullyQualifiedClassNameSet);
        int compileResult;
        try {
            /*
             * Disable sharedNameTable in runtime complication
             *
             * The SharedNameTable was introduced to speed up Java complication by using
             * soft references to avoid re-allocations. However, in fast-avro runtime
             * compilation, sharedNameTable brings severe Memory and GC issue. When
             * fast-avro needed to process a large number of different schemas,
             * SharedNameTable objects will consume huge memory and cannot be freed.
             *
             * SharedNameTable should be disabled for runtime compilation by
             * "-XDuseUnsharedTable" config. The memory issue by SharedNameTable does not
             * exist in Java 11 (tested JDK-11_0_5-zulu and JDK-11_0_5-zing_19_12_100_0_1),
             * thus the change can be reverted in java 11. Keeping this config also does not
             * bring any downgrade.
             *
             */
