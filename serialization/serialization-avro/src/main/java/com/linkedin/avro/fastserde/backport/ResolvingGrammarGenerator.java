...
public class ResolvingGrammarGenerator extends ValidatingGrammarGenerator {
    /**
     * Resolves the writer schema writer and the reader schema reader and returns
     * the start symbol for the grammar generated.
     *
    ...
    /**
     * Resolves the writer schema writer and the reader schema reader and returns
     * the start symbol for the grammar generated. If there is already a symbol in
     * the map seen for resolving the two schemas, then that symbol is returned.
     * Otherwise, a new symbol is generated and returned.
     *
     * @param writer The schema used by the writer
     * @param reader The schema used by the reader
    ...
    public Symbol generate(Schema writer, Schema reader, Map<LitS, Symbol> seen) throws IOException {
        ...
        if (writerType == readerType) {
            switch (writerType) {
                ...
                case FIXED -> {
                    if (AvroCompatibilityHelper.getSchemaFullName(writer)
                ...
                case ENUM -> {
                    ...
                    } else if (AvroCompatibilityHelper.getSchemaFullName(writer) == null || AvroCompatibilityHelper
                            .getSchemaFullName(writer).equals(AvroCompatibilityHelper.getSchemaFullName(reader))) {
                ...
                case ARRAY -> {
                    return Symbol.seq(Symbol.repeat(Symbol.ARRAY_END,
                            generate(writer.getElementType(), reader.getElementType(), seen)), Symbol.ARRAY_START);
                }
                case MAP -> {
                    return Symbol.seq(Symbol.repeat(Symbol.MAP_END,
                            generate(writer.getValueType(), reader.getValueType(), seen), Symbol.STRING), Symbol.MAP_START);
                }
                case RECORD -> {
                ...
                default -> throw new AvroTypeException("Unknown type for schema: " + writerType);
            }
            ...
            switch (readerType) {
                case LONG:
                    if (Objects.requireNonNull(writerType) == Schema.Type.INT) {
                ...
                case DOUBLE:
                    ...
                    break;
                case BYTES:
                    if (Objects.requireNonNull(writerType) == Schema.Type.STRING) {
                        return Symbol.resolve(super.generate(writer, seen), Symbol.BYTES);
                    }
                    break;
                case STRING:
                    if (Objects.requireNonNull(writerType) == Schema.Type.BYTES) {
        ...
        return Symbol.error("Found " + AvroCompatibilityHelper.getSchemaFullName(writer) + ", expecting "
    ...
    @Deprecated
    public static void encode(Encoder e, Schema s, Object o) throws IOException {
        switch (s.getType()) {
            case RECORD -> {
            ...
            case FIXED -> {
                if (!(o instanceof GenericData.Fixed oFixedLengthBytes)) {
    ...
    private static Symbol mkEnumAdjust(List<String> wsymbols, List<String> rsymbols) {
        ...
        for (int i = 0; i < adjustments.length; i++) {
            ...
            adjustments[i] = (j == -1 ? "No match for " + wsymbols.get(i) : Integer.valueOf(j));
        }
    ...
    private static Symbol mkEnumAdjustWithDefault(Schema writer, Schema reader) {
        Avro19Resolver.EnumAdjust e = (Avro19Resolver.EnumAdjust) Avro19Resolver.EnumAdjust.resolve(writer, reader,
                GenericData.get());
     ...
     * fields are present (the symbol is not an error action) and that all the
    ...
    private int bestBranch(Schema r, Schema w, Map<LitS, Symbol> seen) throws IOException {
        ...
        for (Schema b : r.getTypes()) {
            switch (vt) {
                ...
                case LONG, FLOAT -> {
                    if (Objects.requireNonNull(b.getType()) == Schema.Type.DOUBLE) {
                        return j;
                    }
                }
                case STRING -> {
                    if (Objects.requireNonNull(b.getType()) == Schema.Type.BYTES) {
                        return j;
                    }
                }
                case BYTES -> {
                    if (Objects.requireNonNull(b.getType()) == Schema.Type.STRING) {
                        return j;
                    }
                }
                default -> {
     ...
     * {@link ValidatingGrammarGenerator#validating()} from those put in by
     * {@link ValidatingGrammarGenerator#resolving()}.
     */
