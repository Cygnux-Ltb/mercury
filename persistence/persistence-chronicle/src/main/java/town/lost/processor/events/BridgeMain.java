    ...
    public static void main(String[] args) {
        ...
        try (ChronicleQueue queue = ChronicleQueue.singleBuilder("in").sourceId(1).build()) {
            try (ChronicleQueue queue2 = ChronicleQueue.singleBuilder("out").sourceId(2).build()) {
                ...
                Events out = queue2.methodWriterBuilder(Events.class)
                        //.recordHistory(true)
                        .build();
                Events bridge = new BridgeEvents(out);
                MethodReader methodReader = queue.createTailer("bridge").methodReader(bridge);
...
record BridgeEvents(Events out) implements Events {
