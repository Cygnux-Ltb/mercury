...
public class Window<K, R, P> {
    ...
    private final AtomicInteger pendingOffers;
    private final AtomicBoolean pendingOffersAborted;
    ...
    public Window(int size, ScheduledExecutorService executor, long monitorInterval, WindowListener<K, R, P> listener,
        ...
        if (listener != null) {
            this.listeners.add(new UnwrappedWeakReference<>(listener));
        }
    ...
    public Map<K, WindowFuture<K, R, P>> createSortedSnapshot() {
        return new TreeMap<>(this.futures);
    }
