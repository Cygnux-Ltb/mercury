...
public class PerfLongAdaptiveRadixTreeMap {
	...
	private enum Benchmark {
		BST_PUT, BST_GET_HIT, BST_REMOVE, BST_FOREACH, BST_FOREACH_DESC, BST_HIGHER, BST_LOWER, ART_PUT, ART_GET_HIT,
	...
	private BiConsumer<Benchmark, Long> benchmarkConsumer = (b, t) -> times
			.compute(b, (k, v) -> v == null ? new ArrayList<>() : v).add(t);
...
//        final UnaryOperator<Integer> stepFunction = i->1+rand.nextInt(Integer.MAX_VALUE);
		...
		for (int iter = 0; System.currentTimeMillis() < timeEnd && iter < 3000; iter++) {
			log.debug("-------------------iteration:{} ({}s left)------------------------", iter,
...
//                log.debug("CHECK:{} {} ---------", x, String.format("%Xh", x));
				...
				if (!Objects.equals(v1, v2)) {
					...
					System.out.println(art.outputDiagram());
...
//            log.debug("\n{}", art.printDiagram());
			...
			executeInRandomOrder(
					...
					() -> {
						...
						long t = System.nanoTime();
						bst.entrySet().stream().limit(forEachSize)
								.forEach(e -> forEachConsumerBst.accept(e.getKey(), e.getValue()));
...
//            log.debug(" forEach size {} vs {}", forEachKeysArt.size(), forEachKeysBst.size());
			...
			executeInRandomOrder(
					...
					() -> {
						...
						long t = System.nanoTime();
						bst.descendingMap().entrySet().stream().limit(forEachSize)
								.forEach(e -> forEachConsumerBst.accept(e.getKey(), e.getValue()));
...
//        });
			...
			Function<Benchmark, Long> getBenchmarkNs = b -> Math
					.round(times.get(b).stream().mapToLong(x -> x).average().orElse(0));
			...
			log.info("AVERAGE PUT    BST {}ms ADT {}ms ({}%)", nanoToMs(bstPutTimeNsAvg), nanoToMs(artPutTimeNsAvg),
					percentImprovement(bstPutTimeNsAvg, artPutTimeNsAvg));

			log.info("AVERAGE GETHIT BST {}ms ADT {}ms ({}%)", nanoToMs(bstGetHitTimeNsAvg),
					nanoToMs(artGetHitTimeNsAvg), percentImprovement(bstGetHitTimeNsAvg, artGetHitTimeNsAvg));

			log.info("AVERAGE REMOVE BST {}ms ADT {}ms ({}%)", nanoToMs(bstRemoveTimeNsAvg),
					nanoToMs(artRemoveTimeNsAvg), percentImprovement(bstRemoveTimeNsAvg, artRemoveTimeNsAvg));

			log.info("AVERAGE FOREACH BST {}ms ADT {}ms ({}%)", nanoToMs(bstForEachTimeNsAvg),
					nanoToMs(artForEachTimeNsAvg), percentImprovement(bstForEachTimeNsAvg, artForEachTimeNsAvg));

			log.info("AVERAGE FOREACH DESC BST {}ms ADT {}ms ({}%)", nanoToMs(bstForEachDescTimeNsAvg),
					nanoToMs(artForEachDescTimeNsAvg),
					percentImprovement(bstForEachDescTimeNsAvg, artForEachDescTimeNsAvg));

			log.info("AVERAGE HIGHER BST {}ms ADT {}ms ({}%)", nanoToMs(bstHigherTimeNsAvg),
					nanoToMs(artHigherTimeNsAvg), percentImprovement(bstHigherTimeNsAvg, artHigherTimeNsAvg));

			log.info("AVERAGE LOWER BST {}ms ADT {}ms ({}%)", nanoToMs(bstLowerTimeNsAvg), nanoToMs(artLowerTimeNsAvg),
					percentImprovement(bstLowerTimeNsAvg, artLowerTimeNsAvg));
		}
	...
	private static <K, V> void checkStreamsEqual(final Stream<Map.Entry<K, V>> entry,
			final Stream<Map.Entry<K, V>> origEntry) {
		...
		while (iter.hasNext() && origIter.hasNext()) {
			...
			if (!next.getKey().equals(origNext.getKey())) {
				throw new IllegalStateException(
						String.format("unexpected key: %s  (expected %s)", next.getKey(), origNext.getKey()));
			}
			if (!next.getValue().equals(origNext.getValue())) {
				throw new IllegalStateException(
						String.format("unexpected value: %s  (expected %s)", next.getValue(), origNext.getValue()));
			}
		}
