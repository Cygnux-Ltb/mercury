 ...
 * or {@link #threadAssertNull}, (not {@code fail}, {@code assertTrue}, etc.) It
 * is OK (but not particularly recommended) for other code to use these forms
 * too. Only the most typically used JUnit assertion methods are defined this
 * way, but enough to live with.</li>
 * <p>
 * <li>If you override {@link #setUp} or {@link #tearDown}, make sure to invoke
 * {@code super.setUp} and {@code super.tearDown} within them. These methods are
 * used to clear and check for thread assertion failures.</li>
 * <p>
 * <li>All delays and timeouts must use one of the constants {@code
 * SHORT_DELAY_MS}, {@code SMALL_DELAY_MS}, {@code MEDIUM_DELAY_MS},
 * {@code LONG_DELAY_MS}. The idea here is that a SHORT is always discriminable
 * from zero time, and always allows enough time for the small amounts of
 * computation (creating a thread, calling a few methods, etc) needed to reach a
 * timeout point. Similarly, a SMALL is always discriminable as larger than
 * SHORT and smaller than MEDIUM. And so on. These constants are set to
 * conservative values, but even so, if there is ever any doubt, they can all be
 * increased in one spot to rerun tests on slower platforms.</li>
 * <p>
 * <li>All threads generated must be joined inside each test case method (or
 * {@code fail} to do so) before returning from the method. The {@code joinPool}
 * method can be used to do this when using Executors.</li>
 * <p>
 * </ol>
 * <p>
 * <p>
 ...
 * <ul>
 * <p>
 * <li>Usually, there is one testcase method per JSR166 method covering "normal"
 * operation, and then as many exception-testing methods as there are exceptions
 * the method can throw. Sometimes there are multiple tests per JSR166 method
 * when the different "normal" behaviors differ significantly. And sometimes
 * testcases cover multiple methods when they cannot be tested in
 * isolation.</li>
 * <p>
 ...
 * package to simplify things for people integrating them in TCK test
 * suites.</li>
 * <p>
 * <li>As a convenience, the {@code main} of this class (JSR166TestCase) runs
 * all JSR166 unit tests.</li>
 * <p>
 * </ul>
 */
...
public class JSR166TestCase extends TestCase {
	...
	/**
	 * Returns the shortest timed delay. This could be reimplemented to use for
	 * example a Property.
	 */
	...
	/**
	 * Returns a new Date instance representing a time delayMillis milliseconds in
	 * the future.
	 */
	 ...
	 * <p>
	 * Triggers test case failure if any thread assertions have failed, by
	 * rethrowing, in the test harness thread, any exception recorded earlier by
	 * threadRecordFailure.
	 * <p>
		...
		for (int i = 0; i < count; i++) {
			...
			if (name.startsWith("ForkJoinPool-")) {
				...
				throw new AssertionFailedError(
						String.format("Found leaked ForkJoinPool thread test=%s thread=%s%n", toString(), name));
			}
		}
	}
	 ...
	 * threadRecordFailure) any AssertionFailedError thrown, so that the current
	 * testcase will fail.
	 */
	...
	/**
	 * Records the given exception using {@link #threadRecordFailure}, then rethrows
	 * the exception, wrapping it in an AssertionFailedError if necessary.
	 */
	...
	/**
	 * Delays, via Thread.sleep, for the given millisecond delay, but if the sleep
	 * is shorter than specified, may re-sleep or yield until time elapses.
	 */
	...
	/**
	 * Checks that the threads do not terminate within the default millisecond delay
	 * of {@code timeoutMillis()}.
	 */
	 ...
	 * execute the runnable ingoring permisions.
	 */
	...
	/**
	 * Sleeps until the given time has elapsed. Throws AssertionFailedError if
	 * interrupted.
	 */
	...
	/**
	 * Spin-waits up to the specified number of milliseconds for the given thread to
	...
	/**
	 * Waits up to LONG_DELAY_MS for the given thread to enter a wait state:
	...
	/**
	 * Returns the number of milliseconds since time given by startNanoTime, which
	 * must have been previously returned from a call to.
	 */
	 ...
	 * {@link Thread#join(long)}), else interrupts the thread (in the hope that it
	 * may terminate later) and fails.
	 */
