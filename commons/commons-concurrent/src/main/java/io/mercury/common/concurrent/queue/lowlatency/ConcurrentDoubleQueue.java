 ...
 * double[]. This class mimics the interface of
 * {@linkplain java.util.concurrent.BlockingQueue BlockingQueue}, however works
 * with primitive doubles rather than {@link Object}s, so is unable to actually
 * implement the BlockingQueue .
 * <p>
 * This class takes advantage of the Unsafe.putOrderedInt, which allows us to
 * create non-blocking code with guaranteed writes. These writes will not be
 * re-ordered by instruction reordering. Under the covers it uses the faster
 * store-store barrier, rather than the the slower store-load barrier, which is
 * used when doing a volatile write. One of the trade off with this improved
 * performance is we are limited to a single producer, single consumer. For
 * further information on this see, the blog post
 * <a href="http://robsjava.blogspot.co.uk/2013/06/a-faster-volatile.html">A
 * Faster Volatile</a> by Rob Austin.
 * <p>
 * <p>
 * <p>
 ...
 * <p>
 * <p>
 * This is a classic &quot;bounded buffer&quot;, in which a fixed-capacityd
 * array holds elements inserted by producers and extracted by consumers. Once
 * created, the capacity cannot be changed. Attempts to {@link #put put(e)} an
 * element into a full queue will result in the operation blocking; attempts to
 * {@link #take take()} an element from an empty queue will similarly block.
 * <p>
 * <p>
 * Due to the lock free nature of its implementation, ordering works on a first
 * come first served basis.
 * <p>
 * Methods come in four forms, with different ways of handling operations that
 * cannot be satisfied immediately, but may be satisfied at some point in the
 * future: one throws an exception, the second returns a special value (either
 * <tt>null</tt> or <tt>false</tt>, depending on the operation), the third
 * blocks the current thread indefinitely until the operation can succeed, and
 * the fourth blocks for only a given maximum time limit before giving up. These
 * methods are summarized in the following table: <table BORDER CELLPADDING=3
 * CELLSPACING=1>
 * <tr>
 * <td></td>
 ...
 * <td>{@link #offer(double, long, java.util.concurrent.TimeUnit) offer(e, time,
 * unit)}</td>
 * </tr>
 * <tr>
 * <td><b>Remove</b></td>
 ...
 * <p>
 * <p>
 * A <tt>uk.co.boundedbuffer.ConcurrentBlockingDoubleQueue</tt> is capacity
 * bounded. At any given time it may have a <tt>remainingCapacity</tt> beyond
 * which no additional elements can be <tt>put</tt> without blocking.
 * <p>
 * <p>
 * It is not possible to remove an arbitrary element from a queue using
 * <tt>remove(x)</tt>. As this operation would not performed very efficiently.
 * <p>
 * <p>
 * All of <tt>uk.co.boundedbuffer.ConcurrentBlockingDoubleQueue</tt> methods are
 * thread-safe when used with a single producer and single consumer, internal
 * atomicity is achieved using lock free strategies, such as sping locks.
 * <p>
 * <p>
 * Like a <tt>BlockingQueue</tt>, the
 * uk.co.boundedbuffer.ConcurrentBlockingDoubleQueue does <em>not</em>
 * intrinsically support any kind of &quot;close&quot; or &quot;shutdown&quot;
 * operation to indicate that no more items will be added. The needs and usage
 * of such features tend to be implementation-dependent. For example, a common
 * tactic is for producers to insert special <em>end-of-stream</em> or
 * <em>poison</em> objects, that are interpreted accordingly when taken by
 * consumers.
 * <p>
 * <p>
 * Usage example, based on a typical producer-consumer scenario. Note that a
 * <tt>BlockingQueue</tt> can ONLY be used with a single producers and single
 * consumer thead.
 * 
 * <pre>
 *
...
@ThreadSafe
public final class ConcurrentDoubleQueue extends AbstractConcurrentQueue {
	 ...
	 */
	public ConcurrentDoubleQueue(int capacity) {
		super(capacity);
	}
	 ...
	 * currently available. When using a capacity-restricted queue, it is generally
	 * preferable to use {@link #offer(double) offer}.
	 *
	...
	public double take() {
		...
		// purposely not volatile as the read memory barrier occurred above when we read
		// 'writeLocation'
	...
	public double peek(long timeout, TimeUnit unit) throws InterruptedException, TimeoutException {
	 ...
	 * Inserts the specified element into this queue if it is possible to do so
	 * immediately without violating capacity restrictions, returning <tt>true</tt>
	 * upon success and <tt>false</tt> if no space is currently available. When
	 ...
	 * @throws InterruptedException if interrupted while waiting
	 */
	public boolean offer(double value, long timeout, TimeUnit unit) throws InterruptedException {
	 ...
	 * @throws java.util.concurrent.TimeoutException if timeout time is exceeded
	 */
	public double poll(long timeout, TimeUnit unit) throws InterruptedException, TimeoutException {
	 ...
	 * formally, returns <tt>true</tt> if and only if this queue contains at least
	 * one element <tt>e</tt> such that <tt>o.equals(e)</tt>. The behavior of this
	 * operation is undefined if modified while the operation is in progress.
	 *
	 * @param o object to be checked for containment in this queue
	 * @return <tt>true</tt> if this queue contains the specified element
	 * @throws ClassCastException   if the class of the specified element is
	 *                              incompatible with this queue (<a href=
	 *                              "../Collection.html#optional-restrictions">optional</a>)
	 * @throws NullPointerException if the specified element is null (<a href=
	 *                              "../Collection.html#optional-restrictions">optional</a>)
	 */
	 ...
	 * array. If the target array is smaller than the number of elements then the
	 * number of elements read will equal the capacity of the array. This operation
	 * may be more efficient than repeatedly polling this queue. A failure
	 * encountered while attempting to add elements to collection <tt>c</tt> may
	 * result in elements being in neither, either or both collections when the
	 * associated exception is thrown. Further, the behavior of this operation is
	 * undefined if the following methods are called during progress of this
	 * operation {@link #take()}. {@link #offer(double)}, {@link #put(double)},
	 * {@link #drainTo(double[], int)}}
	 *
	 * @param target the collection to transfer elements into
	 * @return the number of elements transferred
	 * @throws UnsupportedOperationException if addition of elements is not
	 *                                       supported by the specified collection
	 * @throws ClassCastException            if the class of an element of this
	 *                                       queue prevents it from being added to
	 *                                       the specified collection
	 * @throws NullPointerException          if the specified collection is null
	 * @throws IllegalArgumentException      if the specified collection is this
	 *                                       queue, or some property of an element
	 *                                       of this queue prevents it from being
	...
	int drainTo(double[] target, int maxElements) {
		...
		// to reduce the number of volatile reads we are going to perform a kind of
		// double check reading on the volatile write location
		int writeLocation = this.writeLocation;

		do {

			// in the for loop below, we are blocked reading unit another item is written,
			// this is because we are empty ( aka capacity()=0)
			// inside the for loop, getting the 'writeLocation', this will serve as our read
			// memory barrier.
