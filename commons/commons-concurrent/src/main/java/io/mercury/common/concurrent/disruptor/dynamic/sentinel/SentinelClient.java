...
public class SentinelClient implements ThreadStatusInfo, ConsumeStatusInfo {
    ...
    private final AtomicInteger totalProduceCount = new AtomicInteger();

    private final AtomicInteger totalConsumeCount = new AtomicInteger();
    ...
    private final List<SentinelListener> listenerList = new ArrayList<>();

    /**
     * 运行中的线程
     */
    private final AtomicInteger runThreadCount = new AtomicInteger();
...
//        }
    ...
    public void start() {
        scheduledThreadPoolExecutor.scheduleAtFixedRate(this::check,
                ((long) windowsLength) * checkInterval,
                ((long) windowsLength) * checkInterval, TimeUnit.MILLISECONDS);
    }
}
