 ...
 * float[]. This class mimics the interface of
 * {@linkplain java.util.concurrent.BlockingQueue BlockingQueue}, however works
 * with primitive floats rather than {@link Object}s, so is unable to actually
 ...
 * <td>{@link #offer(float, long, java.util.concurrent.TimeUnit) offer(e, time,
 * unit)}</td>
 * </tr>
 * <tr>
 * <td><b>Remove</b></td>
 ...
 * <p>
 * <p>
 * A <tt>uk.co.boundedbuffer.ConcurrentBlockingFloatQueue</tt> is capacity
 ...
 * <p>
 * <p>
 * All of <tt>uk.co.boundedbuffer.ConcurrentBlockingFloatQueue</tt> methods are
 ...
 * uk.co.boundedbuffer.ConcurrentBlockingFloatQueue does <em>not</em>
...
@ThreadSafe
public final class ConcurrentFloatQueue extends AbstractConcurrentQueue {
	 ...
	 */
	public ConcurrentFloatQueue(int size) {
		super(size);
	}
	 ...
	 * preferable to use {@link #offer(float) offer}.
	 *
	...
	public float peek(long timeout, TimeUnit unit) throws InterruptedException, TimeoutException {
	 ...
	 * @throws InterruptedException if interrupted while waiting
	 */
	public boolean offer(float value, long timeout, TimeUnit unit) throws InterruptedException {
	 ...
	 * @throws java.util.concurrent.TimeoutException if timeout time is exceeded
	 */
	public float poll(long timeout, TimeUnit unit) throws InterruptedException, TimeoutException {
	 ...
	 * number of elements read will equal the size of the array. This operation may
	 * be more efficient than repeatedly polling this queue. A failure encountered
	 * while attempting to add elements to collection <tt>c</tt> may result in
	 * elements being in neither, either or both collections when the associated
	 * exception is thrown. Further, the behavior of this operation is undefined if
	 * the following methods are called during progress of this operation
	 * {@link #take()}. {@link #offer(float)}, {@link #put(float)},
	 * {@link #drainTo(float[], int)}}
	 *
	...
	int drainTo(float[] target, int maxElements) {
		...
		do {
			...
			// this is because we are empty ( aka size()=0)
